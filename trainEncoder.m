...
if isempty(opts.numSamplesPerWord)
    switch opts.type
        ...
        case {'cnn-supervlag'}
            opts.numSamplesPerWord=1000;
        case {'vlag','cnn-vlag','cnn-vlag-gmm','cnn-superfv'}
            opts.numSamplesPerWord=1000 ;
        case {'bovw'}
            opts.numSamplesPerWord = 200 ;
        case {'vlad','fv','cnn-fv','cnn-gradient-fv'}
            opts.numSamplesPerWord = 1000 ;
        otherwise
            assert(false) ;
    end
...
switch encoder.type
    ...
    case {'cnn-superfv-covariance'}
        ...
        [encoder.means, encoder.covariances, encoder.priors] = ...
            vl_supergmmcovariance(descrs, opts.numWords, opts.codedimention);
    case {'cnn-supervlag'}
        %%under debug
        vl_twister('state', opts.seed) ;
        [encoder.words, encoder.kdtree] = vl_superkmeans(descrs, opts.numWords);
    case {'cnn-superfv'}
        vl_twister('state', opts.seed) ;
        [encoder.supermeans, encoder.supercovariances, encoder.superpriors] = ...
            vl_supergmm(descrs, opts.numWords, opts.codedimention);
    case {'bovw', 'vlad'}
        vl_twister('state', opts.seed) ;
    ...
    case {'vlag', 'cnn-vlag'}
        ...
        encoder.words = vl_kmeans(descrs, opts.numWords, 'verbose', 'algorithm', 'elkan') ;
        encoder.kdtree = vl_kdtreebuild(encoder.words, 'numTrees', 2) ;
        %    [wordtrain,~] = vl_kdtreequery(encoder.kdtree, encoder.words, descrs,'MaxComparisons', 100) ;
        %    [encoder.means, encoder.covariances]=vlag_gmm(descrs,encoder.words,wordtrain);
        %     [encoder.g]=vlag_calg2(encoder.means,encoder.covariances);
